...
package com.evildave
{
    ...
    /**
     * FSM class implements 'standard' game FSM, coupled to external game Cycle event.
     *
     * Pause all state machines by not calling Cycle
     * 
    **/
    public class FSM
    {
...
CONFIG::DEBUG { debug.Assert( IDLE == idFunc || idFunc in fsm_this && fsm_this[idFunc] is Function ); }
...
CONFIG::RELEASE {
            ...
            {   // Something is borked, but we caught the exception
                debug.LogError("FSM.Heartbeat:",fsm_state,fsm_this,e);
                fsm_state = IDLE;
            }
}
        }
